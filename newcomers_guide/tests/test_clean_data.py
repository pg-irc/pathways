...
from newcomers_guide.clean_data import (clean_text, clean_up_email_links,
                                         replace_unicode_newlines)
...
class CleanUpNewlinesTest(TestCase):
    ...
    def test_removes_carriage_returns(self):
        text = 'abc\rdef'
        self.assertEqual(clean_text(text), 'abcdef')

    def test_removes_duplicate_spaces(self):
        text = 'abc  def'
        self.assertEqual(clean_text(text), 'abc def')

    def test_leaves_duplicate_spaces_at_line_start_unchanged(self):
        text = '  abc'
        self.assertEqual(clean_text(text), '  abc')
    ...
    def test_replaces_single_newline_in_two_places_with_spaces(self):
        text = 'abc\ndef\nghi'
        self.assertEqual(clean_text(text), 'abc def ghi')
    ...
    def test_leaves_double_newline_unchanged_after_inline_bullet_character(self):
        text = ('a*bc\n\ndef')
        self.assertEqual(clean_text(text), 'a*bc\n\ndef')

    def test_leaves_double_newline_unchanged_after_inline_number(self):
        text = ('a1.bc\n\ndef')
        self.assertEqual(clean_text(text), 'a1.bc\n\ndef')
    ...
    def test_leaves_newline_unchanged_after_heading(self):
        ...
        self.assertEqual(clean_text(text), 'abc\n# def\nghi')
        text = '# def\nghi'
        self.assertEqual(clean_text(text), '# def\nghi')
    ...
    def test_replaces_newline_with_space_after_star_bullet(self):
        ...
        self.assertEqual(clean_text(text), 'abc\n* def ghi')
        text = '* def\nghi'
        self.assertEqual(clean_text(text), '* def ghi')
    ...
    def test_replaces_newline_with_space_after_numbered_list_item_with_period(self):
        ...
        self.assertEqual(clean_text(text), 'abc\n123. def efg')
        text = '123. def\nefg'
        self.assertEqual(clean_text(text), '123. def efg')
    ...
    def test_replaces_single_newlines_with_space_also_after_punctuation(self):
        text = 'abc,\r\ndef.\r\nghi)\r\njkl'
        self.assertEqual(clean_text(text), 'abc, def. ghi) jkl')
    ...
    def test_leaves_newline_after_heading_unchanged(self):
        ...
        self.assertEqual(clean_text(text), 'previous paragraph. \n# Heading\nBody text.')

    def test_leaves_newline_after_heading_at_the_start_of_string_unchanged(self):
        text = '# Heading\nBody text.'
        self.assertEqual(clean_text(text), '# Heading\nBody text.')
    ...
    def test_realistic_example(self):
        ...
        expected = ('## Try CommonMark\n'
                    ...
                    '[commonmark.js](Web: [https://github.com/jgm/commo...](https://github.com/jgm/commonmark.js)), the '
                    'JavaScript reference implementation.\n'
                    '1. item one\n'
                    '2. item two\n'
                    '   - sublist\n'
...
class CleanUpUrlLinksTest(TestCase):
    def test_replaces_http_link_with_markdown(self):
        ...
        self.assertEqual(clean_text(text), 'abc Web: [http://example.com](http://example.com) def')
    ...
    def test_replaces_https_link_with_markdown(self):
        ...
        self.assertEqual(clean_text(text), 'Web: [https://example.com](https://example.com)')
    ...
    def test_http_link_does_not_truncate_equals_sign(self):
        ...
        self.assertEqual(clean_text(text), 'abc Web: [http://example.com=434](http://example.com=434) def')
    ...
    def test_http_link_does_not_truncate_query(self):
        ...
        self.assertEqual(clean_text(text), 'abc Web: [http://foo.com/find?a=a](http://foo.com/find?a=a) def')

    def test_http_link_does_not_truncate_query_with_ampersand(self):
        text = 'abc http://foo.com/find?a=a&b=b def'
        self.assertEqual(clean_text(text), 'abc Web: [http://foo.com/find?a=a&b=b](http://foo.com/find?a=a&b=b) def')

    def test_http_link_does_not_truncate_path_ending_with_word_character(self):
        text = 'abc http://example.com/search def'
        self.assertEqual(clean_text(text), 'abc Web: [http://example.com/search](http://example.com/search) def')
    ...
    def test_link_at_the_start_of_a_file(self):
        ...
        self.assertEqual(clean_text(text), 'Web: [http://foo.com/find?a=a&b=b](http://foo.com/find?a=a&b=b) def')

    def test_link_at_the_end_of_a_file(self):
        text = 'abc http://foo.com/find?a=a&b=b'
        self.assertEqual(clean_text(text), 'abc Web: [http://foo.com/find?a=a&b=b](http://foo.com/find?a=a&b=b)')
    
    def test_http_link_over_28_characters_are_truncated(self):
        text = 'abc http://google.com/search?source=a&page=b def'
        self.assertEqual(clean_text(text), 'abc Web: [http://google.com/search?sou...](http://google.com/search?source=a&page=b) def')

    def test_http_link_does_not_truncate_long_host_name(self):
        text = 'abc http://excessivelylonghostname.com/search def'
        self.assertEqual(clean_text(text), 'abc Web: [http://excessivelylonghostname.com...](http://excessivelylonghostname.com/search) def')
